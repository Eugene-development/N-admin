...
export async function POST({ request, cookies }) {
	try {
		...
		const token = cookies.get('n_auth_token');

		// Try to call backend logout API (optional, for token invalidation)
		if (token) {
			try {
		...
		cookies.delete('n_auth_token', {
			path: '/',
			domain: cookieDomain
		});

		// Clear user data cookie
		cookies.delete('n_auth_user', {
			path: '/',
			domain: cookieDomain
		});
