<script>
	...
	async function addBrand() {
		...
		try {
			const created = await createShopBrand({
				shop_id: shop.id,
				value: newBrandValue.trim(),
	...
	async function addCity() {
		...
		try {
			const created = await createShopCity({
				shop_id: shop.id,
				city_name: newCityName.trim(),
				is_active: true,
				sort_order: cities.length
...
<!-- Backdrop -->
<div class="fixed inset-0 z-40 bg-black/50" onclick={onCancel}></div>
			...
			<div class="border-b px-6">
				<nav class="flex gap-4">
					<button
						...
						class="border-b-2 py-3 text-sm font-medium transition-colors {activeTab === 'basic'
					...
					{#if shop}
						<button
							...
							class="border-b-2 py-3 text-sm font-medium transition-colors {activeTab ===
							'categories'
								? 'border-indigo-600 text-indigo-600'
								: 'border-transparent text-gray-500 hover:text-gray-700'}"
						>
						...
						<button
							...
							class="border-b-2 py-3 text-sm font-medium transition-colors {activeTab === 'brands'
								? 'border-indigo-600 text-indigo-600'
								: 'border-transparent text-gray-500 hover:text-gray-700'}"
						>
						...
						<button
							...
							class="border-b-2 py-3 text-sm font-medium transition-colors {activeTab === 'cities'
								? 'border-indigo-600 text-indigo-600'
								: 'border-transparent text-gray-500 hover:text-gray-700'}"
						>
						...
						<div class="space-y-4">
							...
							<div>
								...
								<input
									...
									id="slug"
									bind:value={slug}
									class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 font-mono shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none"
									placeholder="nazvanie-magazina"
								/>
								<p class="mt-1 text-xs text-gray-500">
									Генерируется автоматически, если не указывать
								</p>
							</div>
						...
						<div class="space-y-4">
							<div class="flex gap-2">
								<input
									...
									class="flex-1 rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none"
